...
import {
  NurseAppointment,
  NurseAppointmentDocument,
} from '../schemas/nurse-appointment.schema';

@Injectable()
export class NurseAppointmentService {
  constructor(
    @InjectModel(NurseAppointment.name)
    private nurseAppointmentModel: Model<NurseAppointmentDocument>,
  ) {}

  async create(
    createNurseAppointmentDto: Partial<NurseAppointment>,
  ): Promise<NurseAppointment> {
    const createdNurseAppointment = new this.nurseAppointmentModel(
  ...
  async findOne(id: string): Promise<NurseAppointment> {
    const nurseAppointment = await this.nurseAppointmentModel
      .findById(id)
      .exec();
    if (!nurseAppointment) {
  ...
  async update(
    id: string,
    updateNurseAppointmentDto: Partial<NurseAppointment>,
  ): Promise<NurseAppointment> {
    const updatedNurseAppointment = await this.nurseAppointmentModel
      .findByIdAndUpdate(id, updateNurseAppointmentDto, { new: true })
      .exec();
  ...
  async remove(id: string): Promise<NurseAppointment> {
    const deletedNurseAppointment = await this.nurseAppointmentModel
